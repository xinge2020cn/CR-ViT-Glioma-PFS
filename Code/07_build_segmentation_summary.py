...
def main() -> int:
    ...
    if (not np.isfinite(data[["dice_similarity", "hd95_mm"]].to_numpy()).all()
            or not data.dice_similarity.between(0, 1).all() or (data.hd95_mm < 0).any()):
        raise ValueError("Segmentation metrics must be finite and within their valid ranges.")
    ...
    numeric = summary.select_dtypes(include=["number"]).columns.difference(["n", "dice_below_0_80_n"])
