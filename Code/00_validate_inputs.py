...
LEGACY_GROUP_FIELDS = {"risk_group_3dvit", "training_cutoff_3dvit"}


def obsolete_group_fields(frame: pd.DataFrame) -> list[str]:
    """Identify input groups superseded by training-derived model predictions."""
    return sorted(LEGACY_GROUP_FIELDS.intersection(frame.columns))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="External analysis workspace containing the input tables.",
    )
...
def main() -> int:
    ...
    for label, frame in (("main", main_data), ("subset", subset_data)):
        obsolete = obsolete_group_fields(frame)
        record(f"No obsolete ViT grouping fields: {label}", not obsolete,
               ", ".join(obsolete) or "groups are derived from fitted predictions")
    ...
    if MAIN_REQUIRED.issubset(main_data.columns):
        ...
        finite_numeric = np.isfinite(finite_numeric).all()
        record("Required numeric fields are numeric", bool(finite_numeric), "age, KPS, volume, and PFS time")
        valid_event = pd.to_numeric(main_data["event"], errors="coerce").isin([0, 1]).all()
        ...
        time_order = (pd.to_numeric(main_data["pfs_time_months"], errors="coerce") <=
                      pd.to_numeric(main_data["follow_up_time_months"], errors="coerce")).all()
        record("PFS does not exceed follow-up", bool(time_order), "same time origin and unit")
        ...
        if (subset_ids.issubset(main_ids) and not main_data.patient_id.duplicated().any()
                and not subset_data.patient_id.duplicated().any()):
