...
REQUIRED_CODE = {
    ...
    "13_build_cohort_summaries.R",
    "14_validate_analysis_outputs.py",
    "15_build_omics_risk_manifest.py",
}
REQUIRED_UPSTREAM = {
    ...
    "08_run_single_cell_analysis.R",
    "09_validate_upstream_configuration.py",
    "io_utils.py",
    "omics_utils.py",
    "single_cell_helpers.R",
    "10_render_occlusion_panels.py",
    "11_evaluate_segmentation.py",
    "12_run_cell_communication.R",
}
...
def main() -> int:
    ...
    forbidden_directories = [root / name for name in ("Data", "Results", "QA", "Figures", "Working", "inputs")]
    ...
    record("Public-release metadata files are present", all(path.exists() for path in required_docs), "README, workflow, configuration, and dependency files")

    readme_text = (root / "README.md").read_text(encoding="utf-8")
    record(
        "Public-viewing rights statement is present",
        "No open-source license is granted" in readme_text and "all rights are reserved" in readme_text,
        "README states the repository's public-viewing rights status",
    )
