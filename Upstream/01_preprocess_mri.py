...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage", choices=("survival", "segmentation"), default="survival")
...
def process_case(row: pd.Series, config: dict, output_dir: Path, sitk,
                 manifest_dir: Path, stage: str = "survival") -> str | dict:
    ...
    mask_column = data_cfg["reference_mask_column"] if stage == "segmentation" else data_cfg["mask_column"]
    tumor_mask = orient(read_image(input_path(row[mask_column]), sitk), pre_cfg["orientation"], sitk)
    ...
    if safe_id != patient_id:
        raise ValueError("Patient identifiers must use letters, digits, hyphens or underscores.")
    if stage == "segmentation":
        paths = {}
        for (_, column), image in zip(sequence_columns, normalized):
            path = output_dir / f"{safe_id}_{column}.nii.gz"
            sitk.WriteImage(image, str(path), useCompression=True)
            paths[column] = str(path.resolve())
        for key, image in ((data_cfg["reference_mask_column"], resampled_tumor),
                           (data_cfg["brain_mask_column"], resampled_brain)):
            path = output_dir / f"{safe_id}_{key}.nii.gz"
            sitk.WriteImage(sitk.Cast(image > 0, sitk.sitkUInt8), str(path), useCompression=True)
            paths[key] = str(path.resolve())
        paths["preprocessing_stage"] = "registered_normalized_full_volume"
        return paths
...
def main() -> int:
    ...
    if args.stage == "segmentation" and config["data"]["reference_mask_column"] not in frame:
        raise ValueError("Segmentation requires independent manual reference masks.")
    ...
    for index, row in frame.iterrows():
        ...
        processed_paths.append(process_case(row, config, args.output_dir, sitk,
                                            args.manifest.parent.resolve(), args.stage))
    output_manifest = frame.copy()
    if args.stage == "segmentation":
        for column in processed_paths[0]:
            output_manifest[column] = [record[column] for record in processed_paths]
    else:
