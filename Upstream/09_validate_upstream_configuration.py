...
def validate_config(config: dict) -> None:
    ...
    if not 0 <= int(config["project"]["random_seed"]) < 2**32:
        raise SystemExit("The project seed must be an unsigned 32-bit integer.")
    ...
    if config["nnunet"]["folds"] != [0, 1, 2, 3, 4]:
        raise SystemExit("Segmentation requires all five patient-level folds.")
    if str(config["nnunet"]["version"]) != "2.4.2":
        raise SystemExit("The segmentation profile requires nnU-Net 2.4.2.")
